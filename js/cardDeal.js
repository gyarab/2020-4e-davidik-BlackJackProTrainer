//určí velikost okna prohlížeče a hracího pole
let toVw;
let toVh;
let toPx;
let toPx2;
          ...
          if (game.players[id].hands[0].score > 20) {
            ...
            if (double == 1) {
              ...
              if (game.players[id].hands[0].cards[0].value == "ACE" || game.players[id].hands[0].cards[1].value == "ACE" || game.players[id].hands[0].cards[2].value == "ACE") {
                perfectBasicStrategy(id, game.players[id].hands[0].score, true, false, dT, game.players[id].hands[0].count);
              } else {
                perfectBasicStrategy(id, game.players[id].hands[0].score, 0, 0, dT, game.players[id].hands[0].count);
              }
              //
              clearInterval(int);
            }
...
function setBet(p) {
  ...
  // autoBet
  if (autoBetBoolean == true) {
    bal = betting;
    if (bal < 20) {
      bal = 20;
    }
  }
  //nastaví velikost sázky
  player.balance -= bal;
  player.bet += bal;
...
//Rozdá karty všem hráčum na začátku kola
  ...
  if (deck.length < 40) {
    ...
    if (nt == 1) {
      ...
      if (cardcycle == 2) {
        ...
        //během rozdávání se počítá hráčovo skóre, jeho bilance a také stav hry pro strategie
        document.getElementById("cl").innerHTML = "Waiting...";
        cardCount(player.hands[0].cards[cardcycle].value);
        //souřadnice pro kartu
...
function split(id, num) {
  ...
  //rozdělení rukou na dvě a určení souřadnic pro druhou ruku
  ...
  function frame() {
    if (pos >= end) {
      ...
      let decision = perfectBasicStrategy(id, game.players[id].hands[1].score, false, false, dT, game.players[id].hands[1].count, true);
      clearInterval(int);
    } else {
      pos2 -= 1;
      pos = pos + ratio;
...
//liznutí si karty akorát pro hráče co užil split
...
//funkce stay ukončí hráčuv tah a hra pokračuje další postavou na řadě, podle toho jak vypadá hra se určí kdo je na řadě
...
//funkce stay pro hráče, co užil split
